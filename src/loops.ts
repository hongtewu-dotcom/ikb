...
const LOOP_CONTRACTS: readonly LoopContract[] = [
  {
    ...
    gates: ["G0", "G1", "G2", "G3"],
    inputs: [
      requirement("source_refs", true, "ikb-data/.system/sources/<source-id>/{source.json,raw,records.jsonl}", "待分析的原始来源和标准化记录 ID。"),
      ...
      requirement("existing_knowledge", false, "ikb-data/knowledge/<scope> 与 governance", "相关 verified/draft、冲突和待复核条目。"),
    ],
    outputs: [
      requirement("analysis_artifact", true, "ikb-data/.system/runs/<run-id>/artifacts/", "可重放的分析报告，而不是只存在于 prompt 的结论。"),
  ...
  },
  {
    ...
    gates: ["G0", "G4", "G5", "G6"],
    inputs: [
      requirement("task_contract", true, "ikb-data/.system/ledger/events.jsonl → task.created/task.updated", "Task 的 goal、acceptance、type、scope、risk。"),
      requirement("role_and_skill_manifest", true, "src/roles.ts / Agent manifest", "允许的角色、Task 类型和 Skill 白名单。"),
      requirement("context_pack", true, "ikb-data/.system/runs/<run-id>/context-pack.md", "实际交给执行角色的知识和来源上下文。"),
      requirement("plan_and_checkpoint", true, "ikb-data/.system/runs/<run-id>/plan.json 与 Run events", "步骤、当前 checkpoint 和恢复入口。"),
    ...
    outputs: [
      ...
      requirement("run_artifacts", true, "ikb-data/.system/runs/<run-id>/artifacts/ + artifact.created", "草稿、diff、测试、报告和验证证据。"),
      ...
      requirement("knowledge_references", true, "knowledge.query_executed + knowledge.referenced + knowledge.used + knowledge.feedback_recorded", "查询、召回、实际采用和最终效果必须分开记录；实际采用绑定交付 Artifact。"),
  ...
  },
  {
    ...
    inputs: [
      ...
      requirement("completed_runs", true, "ledger events + ikb-data/.system/runs/<run-id>/", "窗口内成功、失败、取消和阻断的 Run 集合。"),
      ...
      requirement("knowledge_usage", true, "knowledge.query_executed + knowledge.referenced + knowledge.used + knowledge.feedback_recorded", "哪些查询无结果，哪些知识只被召回，哪些真正帮助了任务、未采用、缺失或错误。"),
      requirement("previous_proposals", false, "governance/reviews、Plan Pack、改进 Task", "避免重复提出无效规则。"),
    ],
    outputs: [
      requirement("harness_review", true, "ikb-data/.system/cache/reports/ 或 .system/runs/<run-id>/artifacts/", "带统计窗口和证据引用的复盘报告。"),
