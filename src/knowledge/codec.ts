...
export function parseKnowledge(text: string, path: string): KnowledgeRecord {
  ...
  return {
    ...
    revisionHistory: asStringArray(fields.revision_history),
    supersededBy: String(fields.superseded_by ?? "").trim() || undefined,
    ...
    canonicalKey: String(fields.canonical_key ?? "").trim(),
    compilationSchema: String(fields.compilation_schema ?? "").trim(),
    compilationCaseId: String(fields.compilation_case_id ?? "").trim(),
    compilationProductId: String(fields.compilation_product_id ?? "").trim(),
    extractionManifestRef: String(fields.extraction_manifest_ref ?? "").trim(),
    compilationRef: String(fields.compilation_ref ?? "").trim(),
    informationLossRef: String(fields.information_loss_ref ?? "").trim(),
...
export function renderKnowledge(record: KnowledgeRecord): string {
  ...
  return [
    ...
    `revision_history: ${JSON.stringify(record.revisionHistory)}`,
    ...(record.supersededBy ? [`superseded_by: ${JSON.stringify(record.supersededBy)}`] : []),
    ...
    `canonical_key: ${JSON.stringify(record.canonicalKey ?? "")}`,
    `compilation_schema: ${JSON.stringify(record.compilationSchema ?? "")}`,
    `compilation_case_id: ${JSON.stringify(record.compilationCaseId ?? "")}`,
    `compilation_product_id: ${JSON.stringify(record.compilationProductId ?? "")}`,
    `extraction_manifest_ref: ${JSON.stringify(record.extractionManifestRef ?? "")}`,
    `compilation_ref: ${JSON.stringify(record.compilationRef ?? "")}`,
    `information_loss_ref: ${JSON.stringify(record.informationLossRef ?? "")}`,
