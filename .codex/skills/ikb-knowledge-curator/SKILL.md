...
Every new draft uses `quality_version: 4` and carries `product_type`, `compilation_ref`, `fact_refs`, and `questions_answered`. Do not force all knowledge into an execution recipe. Architecture maps keep nodes/edges/version/state; domain packs keep entities/relations/rules/states; entity cards keep fields/relations/lifecycle/invariants; flow cards keep trigger/nodes/edges/exceptions/recovery; decision cards keep context/options/choice/state/rationale/rejected/impact. Only a real `playbook` requires ordered steps, checks, rollback, and stop conditions. The body must contain the typed view or directly include the relevant Compilation sections; a short pointer that leaves all useful facts in an unconsumed Artifact is rejected. `confidence` describes evidence strength; `status` describes lifecycle; `verification` describes real Task, drill, or user confirmation.

For batch extraction, require successful `ikb extraction validate` and `ikb extraction verify` reports. Do not curate from an unvalidated result, mismatched input fingerprint, unhashed excerpt, claim without `fact_refs`, unresolved core obligation, or product missing its type-specific structure. `admit` means the Compilation is publishable; `skip` still retains its fact inventory and Gap/evidence view.
