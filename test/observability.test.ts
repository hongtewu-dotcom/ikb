...
test("observability report separates terminal state, quality and knowledge feedback", () => {
  ...
  store.recordKnowledgeQueryEvent({ contractVersion: "knowledge-usage.v1", mode: "context", runId: run.id, resultCount: 2, zeroResult: false });
  ...
  store.recordKnowledgeEvent("kb-pending", "knowledge.referenced", { taskId: task.id, runId: run.id, status: "draft", verification: "advisory", path: "/private/pending.md", query: "验证" });
  store.recordKnowledgeUsageEvent("kb-test", { contractVersion: "knowledge-usage.v1", taskId: task.id, runId: run.id, artifactId: "artifact-1", purpose: "check", note: "用于检查" });
  ...
  store.recordKnowledgeEvent("kb-pending", "knowledge.feedback_recorded", { contractVersion: "knowledge-usage.v1", taskId: task.id, runId: run.id, outcome: "unused", reviewFinding: "incorrect", reasonCode: "source_conflicts_with_current_state", note: "审阅后未采用", evidenceRefs: ["artifact-1"] });
  ...
  assert.equal(report.version, "ikb-observability-report.v2");
  assert.equal(report.knowledgeUsage.queries.total, 1);
  assert.equal(report.knowledgeUsage.queries.byMode.context, 1);
  assert.equal(report.knowledgeUsage.queries.zeroResult, 0);
  assert.equal(report.knowledgeUsage.references, 2);
  assert.equal(report.knowledgeUsage.distinctKnowledge, 2);
  assert.equal(report.knowledgeUsage.uses, 1);
  assert.equal(report.knowledgeUsage.distinctUsedKnowledge, 1);
  assert.equal(report.knowledgeUsage.useByPurpose.check, 1);
  assert.equal(report.knowledgeUsage.referenceToUseRate, 0.5);
  assert.equal(report.knowledgeUsage.feedback.incorrect, 1);
  assert.equal(report.knowledgeUsage.feedback.unused, 1);
  assert.equal(report.knowledgeUsage.feedbackByReason.boundary_missing, 1);
  assert.equal(report.knowledgeUsage.feedbackByReason.source_conflicts_with_current_state, 1);
  assert.equal(report.knowledgeUsage.feedbackReviewFindings.incorrect, 1);
  assert.equal(report.knowledgeUsage.feedbackCompletionRate, 1);
  assert.equal(report.knowledgeUsage.unresolvedReferences, 0);
  assert.equal(report.knowledgeUsage.unresolvedRuns, 0);
