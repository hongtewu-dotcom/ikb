...
- 当前项目处于未安装开发态。除非用户再次明确要求，不注册 marketplace、不安装插件、不创建 `~/.codex/skills` 或 `~/.codex/agents` 入口。
...
- Codex 只从 `plugins/agent-teams/adapters/codex/` 的物理白名单生成候选包；上游 Claude Skill、command 和 agent 不得进入 Codex 包。
