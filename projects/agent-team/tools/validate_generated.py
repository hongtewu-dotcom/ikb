...
def validate_codex(report: Report) -> None:
    marketplace_root = WORKTREE / "dist" / "codex-marketplace"
    ...
    if not marketplace_path.is_file():
        report.add(
            ...
            message="minimal Codex marketplace has not been generated",
            remediation="Run `make generate-codex`.",
        )
        return
    ...
    try:
        ...
        report.add(
            ...
            harness="codex",
            path=marketplace_path,
            message=f"marketplace JSON parse error: {error}",
            remediation="Regenerate with `make generate-codex`.",
        )
        return

    if marketplace.get("name") != "ikb-agent-team":
        report.add(
            severity="error",
            harness="codex",
            path=marketplace_path,
            message="unexpected marketplace name",
            remediation="The release marketplace name must remain `ikb-agent-team`.",
        )
    ...
    for entry in marketplace.get("plugins", []):
        ...
        try:
            ...
            report.add(
                ...
                harness="codex",
                path=manifest_path,
                message=f"invalid plugin manifest: {error}",
                remediation="Regenerate with `make generate-codex`.",
            )
            continue
        ...
        if manifest.get("skills") != "./skills/":
            report.add(
                ...
                harness="codex",
                path=manifest_path,
                message="manifest does not point at the candidate skill allowlist",
                remediation="Set `skills` to `./skills/` in the Codex manifest source.",
            )
        forbidden_manifest_fields = {"hooks", "agents", "commands", "mcpServers"}
        leaked_fields = forbidden_manifest_fields & set(manifest)
        if leaked_fields:
            report.add(
                severity="error",
                harness="codex",
                path=manifest_path,
                message=f"runtime extensions leaked into manifest: {sorted(leaked_fields)}",
                remediation="Keep the plugin as a skills-only governance layer.",
            )
        ...
        if actual != approved:
            report.add(
                ...
                harness="codex",
                path=skills_dir,
                message=f"skill allowlist mismatch: {sorted(actual)}",
                remediation=f"Candidate must contain exactly {sorted(approved)}.",
            )

        for skill_md in sorted(skills_dir.glob("*/SKILL.md")):
            content = skill_md.read_text(encoding="utf-8")
            fm, _ = parse_frontmatter(content)
            if fm.get("name") != skill_md.parent.name or not fm.get("description"):
                report.add(
                    severity="error",
                    harness="codex",
                    path=skill_md,
                    message="invalid or mismatched skill frontmatter",
                    remediation="Name must match the directory and description must be non-empty.",
                )
            if len(content.encode("utf-8")) > 8 * 1024:
                report.add(
                    ...
                    harness="codex",
                    path=skill_md,
                    message="skill exceeds the 8192-byte injection budget",
                    remediation="Move non-essential detail to a reference file.",
                )
            leaked_terms = sorted(
                term for term in claude_terms | fixed_model_terms if term in content
            )
            if leaked_terms:
                report.add(
                    severity="error",
                    harness="codex",
                    path=skill_md,
                    message=f"host or fixed-model terms leaked: {leaked_terms}",
                    remediation="Keep Claude runtime details and model experiments out of the base Codex package.",
                )
...
def validate_pi(report: Report) -> None:
    """Validate local Pi package manifests and their declared skill resources."""
    packages = WORKTREE / ".pi" / "packages"
    if not packages.is_dir():
        return
    skill_id = re.compile(r"^[a-z0-9]+(?:-[a-z0-9]+)*$")
    for manifest_path in packages.glob("*/package.json"):
        try:
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        except json.JSONDecodeError as exc:
            report.add(severity="error", harness="pi", path=manifest_path, message=f"package.json parse error: {exc}", remediation="Regenerate with `make generate HARNESS=pi`.")
            continue
        pi = manifest.get("pi")
        if not isinstance(pi, dict) or not isinstance(pi.get("skills"), list):
            report.add(severity="error", harness="pi", path=manifest_path, message="missing pi.skills list", remediation="Pi packages must declare each generated Agent Skill in package.json.")
            continue
        for rel in pi["skills"]:
            path = manifest_path.parent / str(rel)
            name = path.parent.name
            if not path.is_file() or not skill_id.fullmatch(name) or len(name) > 64:
                report.add(severity="error", harness="pi", path=manifest_path, message=f"invalid declared Pi skill: {rel!r}", remediation="Regenerate with Pi-valid lowercase hyphenated skill ids.")
        for rel in pi.get("extensions", []):
            if not (manifest_path.parent / str(rel)).is_file():
                report.add(severity="error", harness="pi", path=manifest_path, message=f"missing declared extension: {rel!r}", remediation="Regenerate the package or restore its extension source.")
