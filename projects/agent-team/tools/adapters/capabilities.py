...
CAPABILITIES: dict[str, Capability] = {
    ...
    "codex": Capability(
        ...
        notes="Same SKILL.md format as Claude. Committed marketplace at .agents/plugins/marketplace.json + per-plugin manifest at plugins/*/.codex-plugin/plugin.json (point at source plugins/). Agents use TOML at .codex/agents/. Plugin manifests can bundle native lifecycle hooks. AGENTS.md is walked root->cwd with a 32 KiB cap. Commands map to skills.",
    ),
    "copilot": Capability(
        harness_id="copilot",
    ...
    "pi": Capability(
        ...
        tool_name_case="lowercase",
        bare_model_aliases=False,
        notes="Native Agent Skills and TypeScript package extensions. No built-in subagents, MCP, background bash, or native commands; extension-mediated child Pi processes provide bounded delegation.",
    ),
}
...
TOOL_NAME_MAPS: dict[str, dict[str, str]] = {
    ...
    "pi": {
        ...
        "Grep": "search",
        "Glob": "find files",
        "Agent": "run an isolated child Pi process",
        "Task": "run an isolated child Pi process",
    },
}
