...
Task、Run、Approval 和 Artifact 的每次变更都先追加一条事件，再更新当前状态投影。事件一经写入不可修改和删除。

每条事件至少包含：`event_id`、对象类型与 ID、对象内递增序号、事件类型、操作者、时间、`causation_id`、变更摘要和 payload hash。状态表、日报和周报都可以从事件重建；任何无法关联事件的状态修改都视为数据损坏。
